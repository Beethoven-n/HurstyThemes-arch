...
# let's identify the distro before we do anything
...
function install_theme_hurstythemes() {
    ...
    if [[ -z "$theme" ]]; then
		...
		# because the ES dir is in the home folder, sudo usage is debatable between distros
		[ "$distro" == "RetroPie" ] && {
			sudo git clone "https://github.com/$repo/es-theme-$theme.git" "$destination/$theme"
		} || {
			git clone "https://github.com/$repo/es-theme-$theme.git" "$destination/$theme"
		}
}
...
function disable_script() {
	# oh wait the randomizer is dependent on retropie
	[ "$distro" != "RetroPie" ] && {
		dialog --infobox "Randomizer is only supported on RetroPie systems." 3 53; sleep 1
		return 1
	} 
	dialog --infobox "Disabling theme randomizer" 3 20; sleep 0.5

	# let's do this in a way that's a little more in line with the rest of the code
	# also STOP CAT ABUSE >:(
	# every time you cat into grep a poor kitty cries
	local ifexists=$(grep themerandom.sh /opt/retropie/configs/all/autostart.sh)
	[ "$ifexists" == "themerandom.sh" ] && {
		sed 's/\/home\/pi\/scripts\/themerandom.sh//g' /opt/retropie/configs/all/autostart.sh
	}
}
...
function enable_script() {
	...
	dialog --infobox "Enabling theme randomizer" 3 20 ; sleep 0.5

	# the old method would put themerandom.sh BEFORE the shebang.
	# this would literally prevent autostart.sh from running if you're not careful.
...
function gui_hurstythemes() {
	...
	# check the identity of the distro
	[ "$distro" != "RetroPie" ] && {
		# other distros expect files at $HOME/.emulationstation
		local ES_dir="$HOME/.emulationstation"
		local ES_dir_themes="$ES_dir/themes"
	} || {
		local ES_dir="/etc/emulationstation"
		local ES_dir_themes="$ES_dir/themes"
	}
    while true; do
        local theme
        ...
        case "$choice" in
            U)  #update install script to get new theme listings
								[ "$distro" == "RetroPie" ] && {
									local script_destination="$HOME/RetroPie/retropiemenu"
								} || {
									local script_destination="$HOME/.emulationstation/hursty-themes-menu"
									mkdir -p $script_destination
								}
                cd "$script_destination" 
                ...
                if [[ -f "$script_destination/hurstythemes.sh" ]]; then
                  echo "$script_destination/hurstythemes.sh" > /tmp/errorchecking
                 else
                  mv "hurstythemes.sh.bkp" "hurstythemes.sh"
                fi
								# whoa there cowboy. be real careful about which files you 777.
								# usually none of them need it at all
                chmod 755 "hurstythemes.sh" 
                exit
                ;;
